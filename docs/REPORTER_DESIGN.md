...
1. Every declared reporter receives the following on each per-episode invocation, all as environment-variable-supplied URIs resolvable via `runner/io.py`:
   ...
   | `COGAME_MANIFEST_URI` | R | Yes | ~~The full coworld manifest JSON.~~ **Dropped from the contract by D11 (2026-05-20).** Reporters that need game config read it from the replay instead. |
...
5. ~~**Pass the full manifest,** not a pre-computed subset. Lets reporters dereference `variant_id`, read `results_schema`, and inspect the rest of the manifest if needed.~~ **Superseded by D11 (2026-05-20):** the manifest is no longer passed at runtime. The only real consumer in practice was variant-config lookup, and that data is available from the game-owned replay. (D7 also shelved author-declared `output_schema`, removing one of D2's other rationales for full-manifest access.)
...
| Reporter-side reads | `COGAME_RESULTS_URI`, `COGAME_REPLAY_URI` (renamed from game's `COGAME_SAVE_REPLAY_URI`), `COGAME_LOG_URI`, `COGAME_EPISODE_METADATA_URI` | D2, D11 |
...
### D11 — Drop `COGAME_MANIFEST_URI` from the reporter input contract

- **Date:** 2026-05-20
- **Resolves:** post-v1 reconsideration of the input set declared in D2.

**Decision:**

1. The reporter input contract no longer includes `COGAME_MANIFEST_URI`. The platform does not pass the coworld manifest to reporters at runtime.
2. Any per-episode configuration a reporter needs (e.g., the variant's `game_config`) is available via the game-owned **replay** at `COGAME_REPLAY_URI`. Reporters that need it parse the replay format directly. This intentionally couples those reporters to their game's replay format — acceptable because reporters are coworld-specific build artifacts.
3. Standard input set narrows to: `COGAME_RESULTS_URI`, `COGAME_REPLAY_URI`, `COGAME_LOG_URI` (optional), `COGAME_EPISODE_METADATA_URI`, `COGAME_REPORTER_ID`, plus the write target `COGAME_REPORT_OUTPUT_URI`.

**Rationale:**

- D2 passed the whole manifest because it punted on "what does the reporter actually need?" Looking at the only concrete reporter (`paint_arena_summarizer`), the answer is: `variants[variant_id].game_config.{width,height}` — game config for the variant that ran. Nothing else from the manifest was load-bearing. (D7 already shelved the `results_schema` rationale by making author-declared output schemas a non-feature; D6's pre-validation of results against `game.results_schema` made runtime re-validation redundant.)
- The game's replay is the canonical per-episode artifact for everything the game "knows," including its own config. The paint_arena reference implementation writes `{"config": CONFIG, "player_names": [...], "frames": [...], "results": {...}}` (see `packages/coworld/src/coworld/examples/paintarena/game/server.py` near line 297). So variant config is already preserved in the replay for any game that follows that pattern.
- Reporters are *coworld-specific build artifacts* — the paint_arena summarizer ships in the paint_arena coworld bundle and knows it is reading a paint_arena replay. Coupling to that replay format is acceptable for the same reason it is acceptable for the game image itself: both are owned by the same coworld author.
- Dropping the manifest URI shrinks the env block, removes one platform-side I/O round-trip, and eliminates the "look up the variant in the manifest's `variants[]`" indirection that was a one-liner in every reporter that needed it.

**Consequences:**

- §1 executive summary table (Inputs row) updated.
- §5 item 9 amended to drop `COGAME_MANIFEST_URI` and to note the token-exposure-via-replay carve-out (game-written replays may incidentally include episode-scoped tokens; those secrets are useless after the episode ends, so this is not a real exfiltration concern).
- §7 env block and (former) inputs table updated.
- D2 input table row and rationale item 5 annotated as superseded.
- D10 naming catalogue updated to drop `COGAME_MANIFEST_URI`.
- The platform-side runner no longer mints / sets `COGAME_MANIFEST_URI` for reporter containers. Certification (D5) likewise drops it.
- paint_arena_summarizer updated to read grid dimensions from `replay["config"]["width"]` and `replay["config"]["height"]`, replacing the prior manifest-variants lookup. Test fixtures and the smoke harness updated to provide a replay file instead of a manifest file.
- Reporters that genuinely need broader manifest data (none today) would have to either embed the manifest at build time or get it via a future, narrower channel — but that need has not surfaced and is not anticipated for v1's use cases.

**Explicitly NOT decided (deferred):**

- A cross-game convention for "the game must put its full game_config in the replay." Each game owns its replay format; reporters that read replays know which game they belong to. If a cross-coworld reporter shape ever emerges, it can prescribe a minimum replay convention then.
